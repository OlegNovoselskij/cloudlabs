...
class PumpOperationsService(GeneralService):
    _dao = pump_operations_dao

    def create(self, item: PumpOperation) -> None:
        self._dao.create(item)

    def get_all(self) -> List[PumpOperation]:
        return self._dao.find_all()

    def get_by_id(self, item_id: int) -> PumpOperation:
        return self._dao.find_by_id(item_id)

    def update(self, item_id: int, item: PumpOperation) -> None:
