...
from my_project.auth.dao import (
    ...
    sensors_dao, sensors_type_dao, sopla_dao, users_dao
)
...
class CoordinatesService(GeneralService):
    _dao = coordinates_dao

    def create(self, item: Coordinate) -> None:
        self._dao.create(item)

    def get_all(self) -> List[Coordinate]:
        return self._dao.find_all()

    def get_by_id(self, item_id: int) -> Coordinate:
        return self._dao.find_by_id(item_id)

    def update(self, item_id: int, item: Coordinate) -> None:
