...
class SoplaService(GeneralService):
    _dao = sopla_dao

    def create(self, item: Sopla) -> None:
        self._dao.create(item)

    def get_all(self) -> List[Sopla]:
        return self._dao.find_all()

    def get_by_id(self, item_id: int) -> Sopla:
        return self._dao.find_by_id(item_id)

    def update(self, item_id: int, item: Sopla) -> None:
