...
class SensorsService(GeneralService):
    _dao = sensors_dao

    def create(self, item: Sensor) -> None:
        self._dao.create(item)

    def get_all(self) -> List[Sensor]:
        return self._dao.find_all()

    def get_by_id(self, item_id: int) -> Sensor:
        return self._dao.find_by_id(item_id)

    def update(self, item_id: int, item: Sensor) -> None:
        self._dao.update(item_id, item)

    def delete(self, item_id: int) -> None:
        self._dao.delete(item_id)

    def get_by_type_id(self, type_id: int) -> List[Sensor]:
        return self._dao.find_by_type_id(type_id)